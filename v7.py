...
class SecurePostQuantumDreamCrypto:
    ...
    def secure_decrypt(self, encryption_result: Dict, consciousness_response: str) -> Dict:
...
class EnhancedSecureDreamCryptoGUI:
    ...
    def setup_styles(self):
        ...
        self.fonts = {
            ...
            'code': ('Consolas', 9),
            'mono': ('Courier New', 8),
            'results': ('Segoe UI', 11, 'bold'),  # Larger font for results
            'decryption': ('Segoe UI', 12, 'bold')  # Even larger for decrypted data
        }
...
🤖 Machine Learning Integration: {'✅ Active (Random Forest + LightGBM)' if ML_AVAILABLE else '❌ Not Available'}
...
🧪 Security Testing: Comprehensive vulnerability assessment
    ...
    def create_quick_actions(self, parent):
        ...
        actions = [
            ...
            ("🤖 ML Analysis", lambda: self.notebook.select(3), self.colors['accent_quantum']),
            ("📊 View Performance", lambda: self.notebook.select(5), self.colors['info']),
            ("⚙️ Settings", lambda: self.notebook.select(6), self.colors['accent_quantum'])
        ]
    ...
    def create_encryption_section(self, parent):
        ...
        # Save/Export buttons with view controls
        save_frame = tk.Frame(encrypt_frame, bg=self.colors['bg_secondary'])
        save_frame.pack(fill='x', padx=10, pady=5)
        
        # Left side buttons
        left_buttons = tk.Frame(save_frame, bg=self.colors['bg_secondary'])
        ...
        view_json_btn = tk.Button(left_buttons,
                                text="🔍 View Large",
                                command=self.view_encryption_json_large,
                                bg=self.colors['warning'],
                                fg=self.colors['bg_primary'],
                                font=self.fonts['body'],
                                relief='flat',
                                padx=15, pady=5)
        view_json_btn.pack(side='left', padx=5)
        ...
        self.encryption_results = scrolledtext.ScrolledText(encrypt_frame,
                                                          ...
                                                          bg=self.colors['bg_tertiary'],
                                                          fg=self.colors['text_primary'],
                                                          font=self.fonts['results'],  # Larger font
                                                          state='disabled',
    ...
    def create_decryption_section(self, parent):
        ...
        self.decryption_results = scrolledtext.ScrolledText(decrypt_frame,
                                                          ...
                                                          bg=self.colors['bg_tertiary'],
                                                          fg=self.colors['text_primary'],
                                                          font=self.fonts['decryption'],  # Larger font
                                                          state='disabled',
                                                          wrap='word')  # Word wrapping for better readability
        self.decryption_results.pack(fill='x', padx=10, pady=5)
        
        # View controls for decryption results
        view_controls_frame = tk.Frame(decrypt_frame, bg=self.colors['bg_secondary'])
        view_controls_frame.pack(fill='x', padx=10, pady=5)
        
        view_large_btn = tk.Button(view_controls_frame,
                                 text="🔍 View Large",
                                 command=self.view_decrypted_data_large,
                                 bg=self.colors['accent_dream'],
                                 fg=self.colors['bg_primary'],
                                 font=self.fonts['body'],
                                 relief='flat',
                                 padx=15, pady=5)
        view_large_btn.pack(side='left')
        
        copy_decrypted_btn = tk.Button(view_controls_frame,
                                     text="📋 Copy Data",
                                     command=self.copy_decrypted_data,
                                     bg=self.colors['info'],
                                     fg=self.colors['bg_primary'],
                                     font=self.fonts['body'],
                                     relief='flat',
                                     padx=15, pady=5)
        copy_decrypted_btn.pack(side='right')
...
✅ Data successfully encrypted with consciousness-based security!
"""
    ...
    def view_decrypted_data_large(self):
        """View decrypted data in a large, easy-to-read window"""
        if not hasattr(self, 'last_decrypted_data') or not self.last_decrypted_data:
            messagebox.showwarning("No Data", "No decrypted data available to view.")
            return
        
        # Create large view window
        large_view = tk.Toplevel(self.root)
        large_view.title("🔓 Decrypted Data - Large View")
        large_view.geometry("800x600")
        large_view.configure(bg=self.colors['bg_primary'])
        
        # Center the window
        large_view.transient(self.root)
        large_view.grab_set()
        
        # Title
        title_label = tk.Label(large_view,
                              text="🔓 DECRYPTED DATA",
                              bg=self.colors['bg_primary'],
                              fg=self.colors['success'],
        ...
        # Large text display
        text_frame = tk.Frame(large_view, bg=self.colors['bg_primary'])
        text_frame.pack(fill='both', expand=True, padx=20, pady=10)
        
        large_text = scrolledtext.ScrolledText(text_frame,
                                             bg=self.colors['bg_secondary'],
                                             fg=self.colors['text_primary'],
                                             font=('Segoe UI', 14, 'normal'),  # Large, readable font
                                             wrap='word',
                                             padx=20,
                                             pady=20)
        ...
        large_text.insert('1.0', self.last_decrypted_data)
        large_text.config(state='disabled')
        
        # Buttons
        button_frame = tk.Frame(large_view, bg=self.colors['bg_primary'])
        button_frame.pack(fill='x', padx=20, pady=20)
        
        copy_btn = tk.Button(button_frame,
                           text="📋 Copy to Clipboard",
                           command=lambda: self.copy_text_to_clipboard(self.last_decrypted_data),
                           bg=self.colors['accent_quantum'],
                           fg=self.colors['bg_primary'],
                           font=self.fonts['subheader'],
                           relief='flat',
    ...
    def copy_decrypted_data(self):
        """Copy decrypted data to clipboard"""
        if hasattr(self, 'last_decrypted_data') and self.last_decrypted_data:
            self.copy_text_to_clipboard(self.last_decrypted_data)
            self.status_label.config(text="📋 Decrypted data copied to clipboard", fg=self.colors['success'])
        else:
            messagebox.showwarning("No Data", "No decrypted data available to copy.")
    ...
    def copy_text_to_clipboard(self, text):
        ...
        try:
            ...
            self.status_label.config(text="📋 Text copied to clipboard", fg=self.colors['success'])
        except Exception as e:
    ...
    def perform_decryption(self):
        ...
        try:
            ...
            # Store decrypted data for large view
            self.last_decrypted_data = decryption_result['plaintext']
            
            # Display results with improved formatting
            self.decryption_results.config(state='normal')
            self.decryption_results.delete('1.0', tk.END)
            
            # Create formatted result with emphasis on decrypted data
...
✅ Authentication successful - consciousness pattern verified!
"""
...
"""
            ...
            if 'ml_analysis' in analysis_result:
                ml_data = analysis_result['ml_analysis']
                metrics_text += f"""• ML Classification: {ml_data.get('classification', {}).get('predicted_class', 'unknown').upper()}
• ML Confidence: {ml_data.get('classification', {}).get('confidence', 0):.3f}
• Anomaly Detected: {'YES' if ml_data.get('anomaly_detection', {}).get('is_anomaly', False) else 'NO'}
• ML Security Score: {ml_data.get('security_scoring', {}).get('predicted_score', 0):.3f}
• Risk Level: {ml_data.get('security_scoring', {}).get('risk_level', 'UNKNOWN')}

"""
            
            # Add security assessment
            if 'security_assessment' in analysis_result:
                assessment = analysis_result['security_assessment']
                metrics_text += f"""🔍 SECURITY ASSESSMENT:
• Traditional Score: {assessment.get('traditional_score', 0):.3f}
• ML Enhanced Score: {assessment.get('ml_enhanced_score', 0):.3f}
• Final Score: {assessment.get('final_score', 0):.3f}
• Recommendation: {assessment.get('authentication_recommendation', 'Unknown')}

"""
            
            metrics_text += f"""📈 Mathematical Analysis:
...
• Complexity Bonus: +{analysis_result['mathematical_analysis']['complexity_bonus']:.1f} bits
• Uniqueness Bonus: +{analysis_result['mathematical_analysis']['uniqueness_bonus']:.1f} bits
"""
            
            # Add ML entropy boost if available
            if 'ml_entropy_boost' in analysis_result['mathematical_analysis']:
                metrics_text += f"• ML Entropy Boost: +{analysis_result['mathematical_analysis']['ml_entropy_boost']:.1f} bits\n"
            
            metrics_text += f"""
🔒 Security Assessment:
"""
            
            # Enhanced security level assessment
            if analysis_result['entropy_bits'] >= 50:
                metrics_text += "✅ CRITICAL level security achieved\n"
            elif analysis_result['entropy_bits'] >= 40:
                metrics_text += "✅ SENSITIVE level security achieved\n"
            elif analysis_result['entropy_bits'] >= 30:
                metrics_text += "✅ PERSONAL level security achieved\n"
            elif analysis_result['entropy_bits'] >= 20:
                metrics_text += "⚠️ ENTERTAINMENT level security achieved\n"
            else:
                metrics_text += "❌ Below recommended security levels\n"
            
            # Add ML-based recommendations
            if 'security_assessment' in analysis_result:
                recommendation = analysis_result['security_assessment'].get('authentication_recommendation', '')
                if 'REJECT' in recommendation:
                    metrics_text += "🚨 ML RECOMMENDATION: REJECT - Potential security threat detected\n"
                elif 'CHALLENGE' in recommendation:
                    metrics_text += "⚠️ ML RECOMMENDATION: Additional verification required\n"
                elif 'ACCEPT' in recommendation:
                    metrics_text += "✅ ML RECOMMENDATION: Authentication approved\n"
...
"""
    ...
    def quick_security_test(self):
        ...
        self.notebook.select(4)  # Switch to testing tab
...
"""
    ...
    def view_encryption_json_large(self):
        """View encryption JSON in a large, easy-to-read window"""
        if not self.encryption_result:
            messagebox.showwarning("No Data", "No encryption data available to view.")
            return
        
        # Create large view window
        large_view = tk.Toplevel(self.root)
        large_view.title("🔐 Encryption Data - Large View")
        large_view.geometry("900x700")
        large_view.configure(bg=self.colors['bg_primary'])
        ...
        large_text = scrolledtext.ScrolledText(text_frame,
                                             bg=self.colors['bg_secondary'],
                                             fg=self.colors['text_primary'],
                                             font=('Courier New', 11, 'normal'),  # Monospace for JSON
                                             wrap='word',
                                             padx=15,
                                             pady=15)
        large_text.pack(fill='both', expand=True)
        
        # Insert the JSON data with pretty formatting
        json_text = json.dumps(self.encryption_result, indent=2)
        large_text.insert('1.0', json_text)
        large_text.config(state='disabled')
        ...
        copy_json_btn = tk.Button(button_frame,
                                text="📋 Copy JSON",
                                command=lambda: self.copy_text_to_clipboard(json_text),
                                bg=self.colors['accent_quantum'],
                                fg=self.colors['bg_primary'],
                                font=self.fonts['subheader'],
                                relief='flat',
        ...
        copy_to_decrypt_btn = tk.Button(button_frame,
                                      text="➡️ Send to Decryption",
                                      command=lambda: [self.copy_to_decryption(), large_view.destroy()],
                                      bg=self.colors['success'],
                                      fg=self.colors['bg_primary'],
                                      font=self.fonts['subheader'],
                                      relief='flat',
                                      padx=20, pady=10)
        copy_to_decrypt_btn.pack()
...
class AnimatedSplashScreen:
    ...
    def create_splash_content(self):
        ...
        subtitle_label = tk.Label(main_frame,
                                 text="Mathematical Security + ML Enhanced v2.0",
                                 bg='#0a0a0a',
                                 fg='#a78bfa',
                                 font=('Segoe UI', 14))
        ...
        try:
            if self.animation_step < 40:
                ...
                statuses = [
                    ...
                    "Preparing security frameworks...",
                    "Preparing user interface...",
                    "System ready!"
                ]
                
                status_index = min(self.animation_step // 8, len(statuses) - 1)
                if hasattr(self, 'status_label') and self.status_label.winfo_exists():
                    self.status_label.config(text=statuses[status_index])
                
                # Animate progress bar
                progress_width = int((self.animation_step / 40) * 500)
                ...
                if not self.is_closed:
                    self.splash.after(75, self.animate)
...
def main():
    ...
    print("🚀 Starting Enhanced Secure Post-Quantum Dream Crypto System...")
    print("🔒 Mathematical Security + 🤖 Machine Learning Enhanced")
    print("=" * 80)
    ...
    if ML_AVAILABLE:
        print("✅ Machine Learning capabilities enabled")
        print("   • Random Forest Classification")
        print("   • Isolation Forest Anomaly Detection") 
        print("   • LightGBM Security Scoring")
    else:
        print("⚠️ Machine Learning capabilities disabled")
        print("💡 For enhanced ML features, install:")
        print("   pip install scikit-learn lightgbm joblib")
    ...
    try:
        ...
        # Wait for splash to close with error handling
        try:
            root.wait_window(splash.splash)
        except tk.TclError:
            # Splash window was already destroyed, continue
            pass
    ...
    try:
        ...
        print("👋 Thank you for using Enhanced Secure Post-Quantum Dream Crypto System!")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("🎉 Ready to secure your consciousness data!")
        print("-" * 60)
